...
export async function generateMetadata({
  ...
  return {
    title: `${dict.quiz.nis2.metaTitle} – ${BRAND.name}`,
    ...
    alternates: buildAlternates(lang, "/nis2-check"),
  };
}
...
export default async function Nis2CheckPage({
  ...
  const dict = await getDictionary(lang as Locale);
  const localizedQuiz = getLocalizedQuiz("nis2", dict);
      ...
      <main className="flex-1 max-w-2xl mx-auto w-full px-4 py-10">
        <div className="mb-8">
          <h1 className="text-2xl font-bold text-text mb-2">
            {dict.quiz.nis2.title}
          </h1>
          <p className="text-text-muted">{dict.quiz.nis2.subtitle}</p>
        </div>
